...
class BaseJsonSaver(ABC):
    ...
    @abstractmethod
    def save_json(self, data: list[dict], mode: str, overwrite: bool = False) -> None: ...
...
class JsonSaver(BaseJsonSaver):
    ...
    def save_json(self, data: list[dict], mode: str, overwrite: bool = False) -> None:
        ...
        with open(self.__filename, mode=mode, encoding="utf-8") as f:
            json.dump(data, f, ensure_ascii=False, indent=4)
        if overwrite:
            self.data_vacancy = data
        else:
            self.data_vacancy.extend(data)
    ...
    def add_vacancy(self, vacancies_list: list[Vacancy]) -> None:
        ...
        self.save_json(self.data_vacancy, mode="w", overwrite=True)
